...
pub fn cairo_verify(proof: &CairoProof) -> Result<(), String> {
    ...
    // falls inside the bitwise segment [BITWISE_MEM_BASE,
    // BITWISE_MEM_BASE + BITWISE_PAGE_LEN) must be covered by a
    // bitwise_row whose committed value matches. This closes the gap
    // where a prover could commit bitwise_rows that don't correspond to
    // actual memory the main trace reads/writes. Addresses outside the
    // bitwise page (including high sentinel/padding addresses like those
    // generated by the memory-argument padding) are not our concern.
    {
        use crate::cairo_air::vm::{BITWISE_MEM_BASE, BITWISE_MAX_INV};
        let bw_base = BITWISE_MEM_BASE as u32;
        let bw_end  = bw_base.saturating_add((BITWISE_MAX_INV * 5) as u32);
        for &[addr, val, _mult] in &proof.memory_table_data {
            if addr < bw_base || addr >= bw_end {
                continue;
            }
            let offset = addr - bw_base;
            let row_idx = (offset / 5) as usize;
            let cell = (offset % 5) as usize;
            if row_idx >= proof.bitwise_rows.len() {
                return Err(format!(
                    "memory_table entry at bitwise addr {addr:#x} (row {row_idx}, cell {cell}) \
                     has no corresponding bitwise_row — bitwise memory access \
                     not covered by a committed bitwise invocation"
                ));
            }
            let expected = proof.bitwise_rows[row_idx][cell];
            if val != expected {
                return Err(format!(
                    "bitwise bus link broken at addr {addr:#x}: memory_table value {val:#x} \
                     ≠ bitwise_rows[{row_idx}][{cell}] = {expected:#x}"
                ));
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_bitwise_bus_link_catches_unbacked_memory_entry() {
        // The memory bus link: every memory_table entry at a bitwise
        // address must match the committed bitwise_rows. Inject a bogus
        // entry at a bitwise address and re-seal memory_table_commitment.
        // The verifier's bus-link scan must reject before Fiat-Shamir
        // divergence would catch it via the PoW check.
        ffi::init_memory_pool();
        use crate::cairo_air::vm::BITWISE_MEM_BASE;
        let pairs = vec![(0x12u32, 0x34u32)];
        let mut proof = prove_with_bitwise(&pairs);
        // Inject a memory_table entry at the second bitwise cell (row 0,
        // cell 1 = "y") claiming y = 0xFF, but the committed bitwise_row
        // says y = 0x34.
        let forged_addr = (BITWISE_MEM_BASE + 1) as u32;
        proof.memory_table_data.push([forged_addr, 0xFF, 1]);
        // Recompute memory_table_commitment so the hash check passes and
        // we reach the bus-link scan.
        let mem_flat: Vec<u32> = proof.memory_table_data.iter()
            .flat_map(|&[a, v, m]| [a, v, m])
            .chain(proof.memory_instr_data.iter().flat_map(|&[p, lo, hi, m]| [p, lo, hi, m]))
            .collect();
        proof.memory_table_commitment = crate::channel::hash_words(&mem_flat);
        let err = cairo_verify(&proof).expect_err("unbacked bitwise memory entry must be rejected");
        assert!(err.contains("bitwise bus link") || err.contains("no corresponding bitwise_row"),
            "bus-link rejection should be explicit, got: {err}");
    }
